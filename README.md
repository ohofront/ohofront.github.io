...
The contact form component integrates `EmailJS`.  
EmailJS is a free service that allows you to send emails using JavaScript — without the need for a backend.

To configure your contact form, follow these steps:

1. **Create an EmailJS account** → [https://www.emailjs.com/](https://www.emailjs.com/)

2. In your EmailJS dashboard:

   - Create an **Email Service**, which defines the email provider that will send your messages (e.g., Gmail or Outlook).
   - Create a new **Email Template**, with content like this:
